...
class PyPNMAgent:
    ...
    def _handle_pnm_channel_stats(self, params: dict) -> dict:
        """Get comprehensive channel stats via optimized parallel bulk walks.
        
        This is the new optimized version that:
        1. Does a quick connectivity check (ping + SNMP sysDescr)
        2. Walks all channel stats tables in parallel using bulk_walk_cmd
        3. Returns merged channel stats for SC-QAM, OFDM, ATDMA, and OFDMA
        ...
        self.logger.info(f"Getting channel stats for modem {modem_ip}")
        
        # Step 1: Quick connectivity check (unless skipped)
        if not skip_connectivity_check:
            # Quick ping check (1 packet, 2 second timeout)
            ping_result = subprocess.run(
                ['ping', '-c', '1', '-W', '2', modem_ip],
                capture_output=True,
                text=True
            )
            if ping_result.returncode != 0:
                return {
                    'success': False,
                    'error': f'Modem {modem_ip} not reachable (ping failed)',
                    'reachable': False
                }
            
            # Quick SNMP check - get sysDescr
            snmp_check = asyncio.run(self._async_snmp_get(modem_ip, '1.3.6.1.2.1.1.1.0', community, timeout=3))
            if not snmp_check.get('success'):
                return {
                    'success': False,
                    'error': f'SNMP not responding on {modem_ip}',
                    'reachable': True,
                    'snmp_error': snmp_check.get('error')
                }
        
        # Step 2: Define all table OIDs to walk
        TABLES = {
            # Downstream SC-QAM
        ...
        if not walk_result.get('success'):
            return {
                'success': False,
                'error': 'SNMP table walks failed',
                'details': walk_result.get('error')
            }
        
        raw_results = walk_result.get('results', {})
        walk_time = time.time() - start_time
        
        # Step 4: Parse results into structured data
        def parse_table(table_name: str, base_oid: str, raw_data: list) -> dict:
            """Parse raw SNMP results into per-index structured data."""
            ...
            for item in raw_data:
                ...
                # Format: base_oid.1.column.index
                suffix = oid.replace(base_oid + '.', '').lstrip('.')
                parts = suffix.split('.')
                
                if len(parts) >= 2:
                    try:
                        # Handle .1.col.idx format
                        if parts[0] == '1':
                            col = int(parts[1])
        ...
        # Step 5: Build channel stats response
        ...
        # Build OFDM downstream channels
        ds_ofdm_channels = []
        for idx in sorted(ds_ofdm.keys()):
            ...
            sc_spacing = ofdm.get('subcarrierSpacing', 2)  # 1=25kHz, 2=50kHz
